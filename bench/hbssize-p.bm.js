...
suite.add(`index + index.min.css(as variable)`, function () {
  Handlebars.precompile(IndexHtml);
});

suite.add(`preload`, function () {
  Handlebars.precompile(PreloadHtml);
});
...
suite
  ...
  .on("complete", function done() {
    ...
    const result1 = Handlebars.precompile(IndexHtml);
    console.log("result1.length", result1.length);
    console.timeEnd('index+variable');

    console.time('preload');
    const result2 = Handlebars.precompile(PreloadHtml);
